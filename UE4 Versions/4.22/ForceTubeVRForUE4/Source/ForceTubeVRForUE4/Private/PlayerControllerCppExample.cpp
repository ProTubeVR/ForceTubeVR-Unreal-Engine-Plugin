...
void APlayerControllerCppExample::updateBluetoothSettings() {
	//on S key down (one time per pressure) : 
	if (PlayerInput->GetKeyValue(EKeys::B) && !lastBluetoothSettingsKey) {
		if (GEngine) {
			GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Green, TEXT("Try open android bluetooth settings."));
		}
		UForceTubeVRFunctions::OpenAndroidBluetoothSettings(); // combination of kick and rumble methods ; rumble duration still be in seconds and still don't stop if you set this parameter at 0.0f
	}
	lastBluetoothSettingsKey = PlayerInput->GetKeyValue(EKeys::B);
}