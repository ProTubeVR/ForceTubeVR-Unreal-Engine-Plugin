...
The ForceTubeVR itself has to be paired to Windows via bluetooth standard connection.
...
If a forcetube is already paired on Windows and powered, then all the forcetubevrs will pass from "paired" status to "connected" status (can see it in Windows Bluetooth manager) just after you call InitAsync.
